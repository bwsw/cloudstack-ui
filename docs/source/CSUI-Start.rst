...
Custom scripts are added at running the CloudStack-UI Docker-container. All that a user should do is to define the ```/scripts/custom.js``` custom script content and map it via a Docker-volume. 

By default, the ```/scripts/custom.js``` file is empty.
