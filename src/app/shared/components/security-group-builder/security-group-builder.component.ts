...
export class SecurityGroupBuilderComponent implements OnInit {
  ...
  private initRulesList(): void {
    ...
    for (let i = 0; i < this.securityGroups.selected.length; i++) {
      ...
      for (let j = 0; j < group.ingressrule.length; j++) {
        const ind = this.inputRules.ingress.findIndex(rule => {
          return rule.ruleid === group.ingressrule[j].ruleid;
        });
        this.pushIngressRule(group.ingressrule[j], ind !== -1, NetworkRuleType.Ingress);
      }

      for (let j = 0; j < group.egressrule.length; j++) {
        const ind = this.inputRules.egress.findIndex(rule => {
          return rule.ruleid === group.egressrule[j].ruleid;
        });
        this.pushEgressRule(group.egressrule[j], ind !== -1, NetworkRuleType.Egress);
      }
    }
  }
