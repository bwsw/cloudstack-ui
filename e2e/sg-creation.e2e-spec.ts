...
describe('e2e-test-sg-creation', () => {
  ...
  it('Create custom firewall template', () => {
    ...
    expect(page.checkSelectedTemplatesNotEmpty).toBeFalsy(
      '"Selected templates" list must be empty',
    ); // check "selected templates" is empty
    browser.sleep(2000);
    page.clickSelectAllButton(); // click SELECT ALL
    browser.sleep(2000);
    expect(page.checkAllTemplatesNotEmpty).toBeFalsy(
      '"All templates" list must be empty after click "Select all" button',
    );
    browser.sleep(2000);
    expect(page.checkNetworkRulesNotEmpty).toBeTruthy(
      '"Network rules" list must be filled after click "Select all" button',
    );
    browser.sleep(2000);
    ...
    browser.sleep(2000);
    expect(page.checkNetworkRulesNotEmpty).toBeFalsy(
      '"Network rules" list must be empty after click "Reset" button',
    );
  });
